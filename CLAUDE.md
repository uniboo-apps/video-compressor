...
### ⚠️ マルチスレッドコア（core-mt）は採用しない＝逆に遅い
`@ffmpeg/core-mt`＋COOP/COEP（`_headers`）で cross-origin isolation を有効化して試したが、
x264 エンコードでは **ST より大幅に遅くなった**（同一マシンで同じ22秒クリップ: ST 13秒 → MT 2分超で未完了。2026-06 本番実測）。
wasm の pthread 同期オーバーヘッドが x264 のスレッド処理で支配的になるため。**再挑戦しないこと。**
（MT が効くのは一部フィルタ等。汎用の x264 圧縮では ST が速い。）
